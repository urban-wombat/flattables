...
`gotflat` also generates a Go main program with constructor and getter methods specific to your FlatBuffes schema.
...
Here is the GoDoc of all 90 or so Go functions generated by the Google `flatc` utility and gotables `gotflat` utility (calls `flatc`):
...
```
    $ gotflat -f ../flattables_sample/tables.got -n flattables_sample -p github.com/urban-wombat/flattables_sample
